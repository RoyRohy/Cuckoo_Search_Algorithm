...
#define SETUPPARAMS_H
  ...
  class SetUpParams
  {
	...
	private:
		unsigned int   _independent_runs;         //number of independent runs nombre d'appels à l'algorithm
		unsigned int   _nb_evolution_steps;       // number of iterations per run nombre d'appel à la fitness
		unsigned int   _population_size;		// number of solutions in the population
		unsigned int   _solution_size;	        // size of each particle = dimension du problème
